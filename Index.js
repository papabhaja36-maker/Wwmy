...
// 馃専 SECURE FIREBASE URL FROM GITHUB SECRETS 馃専
...
async function startBot() {
    if (!FIREBASE_URL) {
        console.log("鉂� ERROR: FIREBASE_URL is missing in GitHub Secrets!");
        process.exit(1);
    }
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (qr) {
            console.clear(); 
            console.log('\n==================================================');
            console.log('鈿狅笍 QR CODE TOO BIG? CLICK "View raw logs" in top right!');
            console.log('==================================================\n');
            qrcode.generate(qr, { small: true }); 
        }

        if (connection === 'open') console.log('鉁� JAVAGOAT AI IS ONLINE!');
        if (connection === 'close') {
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        console.log(`馃摡 Query: ${text}`);

        // --- 馃洅 STEP 2: FINISH ORDER & SEND TO ADMIN PANEL ---
        if (orderStates[sender]?.step === 'WAITING_FOR_ADDRESS') {
            ...
            await sock.sendMessage(sender, { text: `鉁� *Order Placed Successfully!* \n\nThank you! Your order for *${item.name}* is being prepared. \n\n*Total:* 鈧�${javaGoatOrder.total} (Inc. Delivery)\n*Status:* Preparing\n\nWe will deliver it to your address soon.` });
            delete orderStates[sender]; 
            return;
        }

        // --- 馃専 STEP 1: START ORDER FLOW (WITH IMAGE & PHONE REQUEST) ---
        if (text.startsWith("order ")) {
            ...
            if (!matchedItem) {
                await sock.sendMessage(sender, { text: `鉂� Sorry, we couldn't find *${productRequested}* in our menu today.\n\nType *menu* to see all available items.` });
                return;
            }

            orderStates[sender] = { step: 'WAITING_FOR_ADDRESS', item: matchedItem };
            
            // 馃専 NEW: SEND PRODUCT IMAGE + ASK FOR PHONE NUMBER 馃専
            const captionText = `馃洅 *Order Started!* \n\nYou selected: *${matchedItem.name}* (鈧�${matchedItem.price})\n\nPlease reply with your *Full Name, Phone Number, and Delivery Address*.`;
            ...
            await sock.sendMessage(sender, { text: "馃洅 *How to order:* \nPlease type 'order' followed by the dish name. \nExample: *order pizza*" });
        }
        ...
        // --- DYNAMIC MENU FEATURE ---
            ...
            let menuMessage = "馃崝 *JAVAGOAT LIVE MENU* 馃崟\n\n";
            currentMenu.forEach(item => {
                menuMessage += `馃敻 *${item.name}* - 鈧�${item.price}\n`;
            });
